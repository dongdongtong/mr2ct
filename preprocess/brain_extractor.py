...
class BrainExtractor(object):
    ...
    def binarize_img(self, img, itk_img, th):
        binary_img = np.zeros_like(img).astype(np.int64)
    ...
    def get_brain_mask(self, itk_img, use_percentile=False):
        img = sitk.GetArrayFromImage(itk_img)

        if use_percentile:
            th = np.percentile(img, 60)
            print("BrainExtractor get_brain_mask threshold: ", th)
        else:
            th = -500
        # print((img == 0).sum())
        ...
        # 2. fill lcc at 1 axis
        img_lcc_arr = img_lcc_arr_filled.copy()
        for i in enumerate(range(img_lcc_arr.shape[1])):
            img_slice = img_lcc_arr[:, i, :]
        ...
        # 3. fill lcc at 2 axis
        img_lcc_arr = img_lcc_arr_filled.copy()
        for i in enumerate(range(img_lcc_arr.shape[2])):
            img_slice = img_lcc_arr[:, :, i]
    ...
    def get_brain_mask_no_binarize(self, binary_img):
        largest_cc_img_mask = self._get_largest_connected_component(binary_img)

        # fill lcc at three axis
        # 1. fill lcc at 0 axis
        img_lcc_arr = sitk.GetArrayFromImage(largest_cc_img_mask)
        img_lcc_arr_filled = np.zeros(img_lcc_arr.shape).astype(np.uint8)
        for i, img_slice in enumerate(img_lcc_arr):
    ...
    def get_seg_mask(self, seg):
        largest_cc_img_mask = self._get_largest_connected_component(seg)
        img_lcc_arr = sitk.GetArrayFromImage(largest_cc_img_mask)
        img_lcc_arr_filled = np.zeros(img_lcc_arr.shape).astype(np.uint8)
        for i, img_slice in enumerate(img_lcc_arr):
            img_lcc_arr_filled[i] = self._fill_hole(img_slice[:, :, None].astype(np.uint8))
