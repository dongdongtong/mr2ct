...
def skull_stripping(input_img_path=None, output_img_path=None, gpu=False, threads=2, border=1, model_path="data/synthstrip/synthstrip.1.pt"):
    if gpu:
    ...
    # load model weights
    # modelfile = os.path.join('scripts', f'synthstrip.1.pt')
    # print(model_path)
    checkpoint = torch.load(model_path, map_location=device)
    model.load_state_dict(checkpoint['model_state_dict'])

    # load input volume
    image = sf.load_volume(input_img_path)
    # print(f'Input image read from: {input_img_path}')
    ...
    if output_img_path is not None:
        ...
        # mask.save(output_img_path)

        output_mask_path = output_img_path.replace(".nii.gz", "_brainmask.nii.gz")
        image.new(mask).save(output_mask_path)
        print(f'Binary brain mask and bet image saved to: {output_mask_path}')
        print(f'Masked image saved to: {output_img_path}')


# multiple processing using skull_stripping function
def multi_process_pool(data_paths, output_paths):
    import multiprocessing as mp
    from functools import partial
    # pool = mp.Pool(mp.cpu_count())
    pool = mp.Pool(18)
    pool.starmap(skull_stripping, zip(data_paths, output_paths))
    # pool.join()
    pool.close()

    
if __name__ == '__main__':
    # input_path = "./train_LUFA_0000.nii.gz"
    # output_path = './train_LUFA_0000_skullstrip_mask.nii.gz'
    # skull_stripping(input_path, output_path, gpu=True)

    data_root_path = "data/ct_reg2_mr_betneck/ct"
    from glob import glob
    from os.path import join, basename, dirname
    data_paths = glob(join(data_root_path, "*.nii.gz"))
    data_paths = [path for path in data_paths if "mask" not in basename(path)]
    output_root_path = "data/ct_reg2_mr_betneck/bet_ct/"
    output_paths = [join(output_root_path, basename(path)) for path in data_paths]

    multi_process_pool(data_paths, output_paths)