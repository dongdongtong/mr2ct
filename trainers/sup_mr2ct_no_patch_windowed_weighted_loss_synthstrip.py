...
class TranslationTrainer:
    ...
    def get_windowed_loss(self, fake_ct, ct_img, head_mask, brain_mask, mode="train"):
        with torch.no_grad():
            head_mask = head_mask.type(torch.BoolTensor).cuda()
            b, c, h, w, d = ct_img.shape
            head_mask_sum = head_mask.view(b, -1).sum(1)

            brain_mask = brain_mask.type(torch.BoolTensor).cuda()
            brain_mask_sum = brain_mask.view(b, -1).sum(1)

            skull_mask = head_mask & ~brain_mask
            skull_mask_sum = skull_mask.view(b, -1).sum(1)

            window_100_1500_mask = (ct_img >= 100) & (ct_img <= 1500)

            skull_rest_mask = skull_mask & ~window_100_1500_mask

        fake_ct_window_0_100 = torch.clamp(fake_ct, 0, 100)
        ct_img_window_0_100 = torch.clamp(ct_img, 0, 100)
        ...
        if mode == "train":
            ...
            l1_loss_window_0_100 = (
                weghted_criterion(
                    fake_ct_window_0_100, 
                    ct_img_window_0_100
                ) * brain_mask).reshape(b, -1).sum(1) / (brain_mask_sum + 1)
        else:
            l1_loss_window_0_100 = (F.l1_loss(fake_ct, ct_img) * brain_mask).reshape(b, -1).sum(1) / (brain_mask_sum + 1)
        l1_loss_window_0_100 = l1_loss_window_0_100.mean()
        loss_ssims_window_0_100 = self.get_ssim_loss(fake_ct_window_0_100, ct_img_window_0_100, brain_mask)
        ...
        log_loss = dict()
        log_loss[f"loss_G/{mode}_l1_loss_window_0_100"] = l1_loss_window_0_100.detach()
        log_loss[f"loss_ssim_3d_unscaled/{mode}_window_0_100"] = loss_ssims_window_0_100['loss_ssim_3d_unscaled'].detach()
        log_loss[f"loss_ssim_yz_unscaled/{mode}_window_0_100"] = loss_ssims_window_0_100['loss_ssim_yz_unscaled'].detach()
        log_loss[f"loss_ssim_xz_unscaled/{mode}_window_0_100"] = loss_ssims_window_0_100['loss_ssim_xz_unscaled'].detach()
        log_loss[f"loss_ssim_xy_unscaled/{mode}_window_0_100"] = loss_ssims_window_0_100['loss_ssim_xy_unscaled'].detach()
        log_loss[f"loss_total/{mode}_window_0_100_loss"] = loss_window_0_100.detach()

        # log_loss[f"loss_G/{mode}_l1_loss_window_100_500"] = l1_loss_window_100_500.detach()
        # log_loss[f"loss_ssim_3d_unscaled/{mode}_window_100_500"] = loss_ssims_window_100_500['loss_ssim_3d_unscaled'].detach()
        # log_loss[f"loss_ssim_yz_unscaled/{mode}_window_100_500"] = loss_ssims_window_100_500['loss_ssim_yz_unscaled'].detach()
        # log_loss[f"loss_ssim_xz_unscaled/{mode}_window_100_500"] = loss_ssims_window_100_500['loss_ssim_xz_unscaled'].detach()
        # log_loss[f"loss_ssim_xy_unscaled/{mode}_window_100_500"] = loss_ssims_window_100_500['loss_ssim_xy_unscaled'].detach()
        ...
        return {
            ...
            # "loss_window_100_500": loss_window_100_500,
            # "l1_loss_window_100_500": l1_loss_window_100_500,
            # "loss_ssims_window_100_500": loss_ssims_window_100_500,
            "loss_window_100_1500": loss_window_100_1500,
            "l1_loss_window_100_1500": l1_loss_window_100_1500,
            # "loss_ssims_window_500_1500": loss_ssims_window_500_1500,
        }
    ...
    def run_train_epoch(self, train_loader):
        ...
        for idx, batch in enumerate(train_loader):
            ...
            ct_skullstrip_mask = batch['ct_skullstrip_mask']
            ...
            if not self.amp:
                ...
                windowed_loss_dict = self.get_windowed_loss(fake_ct, ct_img, ct_brain_mask, ct_skullstrip_mask, "train")
    ...
    def run_eval_epoch_loss(self, val_ds):
        ...
        # window_100_500_loss = AverageMeter()
        # window_100_500_loss_l1 = AverageMeter()
        # window_100_500_loss_ssim_3d_unscaled = AverageMeter()
        # window_100_500_loss_ssim_yz_unscaled = AverageMeter()
        # window_100_500_loss_ssim_xz_unscaled = AverageMeter()
        # window_100_500_loss_ssim_xy_unscaled = AverageMeter()

        window_100_1500_loss = AverageMeter()
        window_100_1500_loss_l1 = AverageMeter()
        # window_500_1500_loss_ssim_3d_unscaled = AverageMeter()
        # window_500_1500_loss_ssim_yz_unscaled = AverageMeter()
        # window_500_1500_loss_ssim_xz_unscaled = AverageMeter()
        # window_500_1500_loss_ssim_xy_unscaled = AverageMeter()
        ...
        for i in tqdm(range(len(val_ds))):
            ...
            if monai.__version__ >= "1.0.0":
                ...
                ct_skullstrip_mask = images['ct_skullstrip_mask'].unsqueeze(0).cuda(self.local_rank)
            else:
                mr_img = torch.from_numpy(images['mr_image']).unsqueeze(0).cuda(self.local_rank)
                ct_img = torch.from_numpy(images['ct_image']).unsqueeze(0).cuda(self.local_rank)
                ct_brainmask = torch.from_numpy(images['ct_brainmask']).unsqueeze(0).cuda(self.local_rank)
                ct_skullstrip_mask = torch.from_numpy(images['ct_skullstrip_mask']).unsqueeze(0).cuda(self.local_rank)
            ...
            loss_l1 = F.l1_loss(fake_ct, ct_img)
            ...
            windowed_loss_dict = self.get_windowed_loss(fake_ct, ct_img, ct_brainmask, ct_skullstrip_mask, "val")
            ...
            # window_100_500_loss.update(windowed_loss_dict['loss_window_100_500'].item(), n=1)
            # window_100_500_loss_l1.update(windowed_loss_dict['l1_loss_window_100_500'].item(), n=1)
            # window_100_500_loss_ssim_3d_unscaled.update(windowed_loss_dict['loss_ssims_window_100_500']['loss_ssim_3d_unscaled'].item(), n=1)
            # window_100_500_loss_ssim_yz_unscaled.update(windowed_loss_dict['loss_ssims_window_100_500']['loss_ssim_yz_unscaled'].item(), n=1)
            # window_100_500_loss_ssim_xz_unscaled.update(windowed_loss_dict['loss_ssims_window_100_500']['loss_ssim_xz_unscaled'].item(), n=1)
            # window_100_500_loss_ssim_xy_unscaled.update(windowed_loss_dict['loss_ssims_window_100_500']['loss_ssim_xy_unscaled'].item(), n=1)
            ...
            # window_500_1500_loss_ssim_3d_unscaled.update(windowed_loss_dict['loss_ssims_window_500_1500']['loss_ssim_3d_unscaled'].item(), n=1)
            # window_500_1500_loss_ssim_yz_unscaled.update(windowed_loss_dict['loss_ssims_window_500_1500']['loss_ssim_yz_unscaled'].item(), n=1)
            # window_500_1500_loss_ssim_xz_unscaled.update(windowed_loss_dict['loss_ssims_window_500_1500']['loss_ssim_xz_unscaled'].item(), n=1)
            # window_500_1500_loss_ssim_xy_unscaled.update(windowed_loss_dict['loss_ssims_window_500_1500']['loss_ssim_xy_unscaled'].item(), n=1)
        ...
        if self.local_rank == 0:
            ...
            # log_losses['loss_G/val_window_100_500'] = window_100_500_loss_l1.avg
            # log_losses['loss_ssim_3d_unscaled/val_window_100_500'] = window_100_500_loss_ssim_3d_unscaled.avg
            # log_losses['loss_ssim_yz_unscaled/val_window_100_500'] = window_100_500_loss_ssim_yz_unscaled.avg
            # log_losses['loss_ssim_xz_unscaled/val_window_100_500'] = window_100_500_loss_ssim_xz_unscaled.avg
            # log_losses['loss_ssim_xy_unscaled/val_window_100_500'] = window_100_500_loss_ssim_xy_unscaled.avg
            # log_losses['loss_total/val_window_100_500'] = window_100_500_loss.avg

            log_losses['loss_G/val_window_100_1500'] = window_100_1500_loss_l1.avg
            # log_losses['loss_ssim_3d_unscaled/val_window_500_1500'] = window_500_1500_loss_ssim_3d_unscaled.avg
            # log_losses['loss_ssim_yz_unscaled/val_window_500_1500'] = window_500_1500_loss_ssim_yz_unscaled.avg
            # log_losses['loss_ssim_xz_unscaled/val_window_500_1500'] = window_500_1500_loss_ssim_xz_unscaled.avg
            # log_losses['loss_ssim_xy_unscaled/val_window_500_1500'] = window_500_1500_loss_ssim_xy_unscaled.avg
