...
def do_mr_to_pct(input_mr_file, output_pct_file, saved_model, device, prep_t1, plot_mrct):
    ...
    if prep_t1:
        ...
        pre_mr_path = os.path.splitext(input_mr_file)[0] + '_prep.nii'
    else:
        pre_mr_path = input_mr_file
        
    mr_tensor = t1trans_test(pre_mr_path).unsqueeze(0)
    ...
    with torch.no_grad():
        pesudo_ct_tensor = do_inference_3D(net, mr_tensor, device)
        pesudo_ct_tensor = torch.from_numpy(pesudo_ct_tensor)
    
    
    # invert transform using monai
    print('Invert the pCT to the original MR space...')
    mr_tensor = mr_tensor.squeeze(0)
    if len(pesudo_ct_tensor.shape) == 3:
        pesudo_ct_tensor = pesudo_ct_tensor.unsqueeze(0)
    elif len(pesudo_ct_tensor.shape) == 5:
        pesudo_ct_tensor = pesudo_ct_tensor.squeeze(0)
    
    assert len(pesudo_ct_tensor.shape) == 4, "pesudo_ct_tensor should have 4 dimensions C, W, H, D"

    # for tf in custom_transforms[::-1]:
    #     pesudo_ct_tensor = tf.inverse(pesudo_ct_tensor)
    ct_out = inverse_transforms(pesudo_ct_tensor, t1trans_test, mr_tensor, pre_mr_path, output_pct_file)
    ...
    if plot_mrct:
        ...
        orig_t1_arr, _ = LoadImage(pre_mr_path)
