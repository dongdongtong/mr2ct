...
class AdjustCTValuesd(transforms.transform.MapTransform, InvertibleTransform):
    ...
    def adjust_values(self, image_array):
        if monai.__version__ >= "1.0.0":
            ...
            image_array = torch.clamp(image_array, -1024, max=2000)
            # image_array += 1024
            # image_array[image_array < -1024] = -1024
        else:
            # monai version is less than 1.0.0
            image_array = np.clip(image_array, a_min=-1024, a_max=2000)
            # image_array += 1024
            # image_array[image_array < -1024] = -1024
...
def get_train_transforms(cfg):
    ...
    train_transforms = [
        ...
        transforms.HistogramNormalized(keys=['mr_image'], min=-1.0, max=1.0),
    ...
    intensity_transforms = [
        ...
        transforms.RandGaussianSmoothd(keys=['mr_image',], prob=0.15),
    ]
