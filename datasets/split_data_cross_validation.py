...
data_root_dir = "data/orig_dcm_data/preprocessed"
ct_dir = os.path.join(data_root_dir, "ct_reg2_mr_betneck")
mr_dir = os.path.join(data_root_dir, "mr_betneck")

# Collect file names in the ct subdir
ct_files = os.listdir(ct_dir)

# Retrieve the same filename in the mr subdir
mr_files = [os.path.join(mr_dir, file) for file in ct_files if "mask" not in basename(file)]
ct_files = [os.path.join(ct_dir, file) for file in ct_files if "mask" not in basename(file)]
...
cross_validation_json_dir = "data/cross_validation_t12ct"
    ...
    for ct_file, mr_file in train_data:
        train_data_json.append(
            {"ct_image": ct_file, 
             ...
             "ct_skullstrip_mask": ct_file.replace('.nii.gz', '_skullstrip_mask.nii.gz'),
             })
