...
def do_prep_aligned_ct_mr(ct_file, out_root_dir):
    ...
    out_ct_file = join(out_root_dir, "bet_ct", fn)
    out_mr_file = join(out_root_dir, "bet_mr", fn)

    bet_ct_img_pre = ants.image_read(out_ct_file)
    bet_ct_brainmask = ants.image_read(out_ct_file.replace('.nii.gz', '_skullstrip_mask.nii.gz'))
    
    head_ct_dir = join(dirname(dirname(out_ct_file)), "ct")
    head_ct_img_pre = ants.image_read(join(head_ct_dir, basename(out_ct_file)))
    head_ct_mask = ants.image_read(join(head_ct_dir, basename(out_ct_file).replace('.nii.gz', '_brainmask.nii.gz')))
    

    bet_mr_img_pre = ants.image_read(out_mr_file)
    head_mr_dir = join(dirname(dirname(out_mr_file)), "mr")
    head_mr_img_pre = ants.image_read(join(head_mr_dir, basename(out_mr_file)))
    ...
    reg_ct_dir = join(out_root_dir, "ct_reg2_mr_based_on_betimg")
    ...
    reg_ct_brainmask_dir = join(out_root_dir, "ct_reg2_mr_based_on_betimg")
    ...
    ants.image_write(reged_ct_headmask, reg_ct_brainmask_file)

    # apply the affine matrix to the skull stripped mask
    reged_bet_ct_brainmask = ants.apply_transforms(
        fixed=head_mr_img_pre, 
        moving=bet_ct_brainmask, 
        transformlist=out_dict['fwdtransforms'],
        interpolator='nearestNeighbor', defaultvalue=0)
    
    reg_bet_ct_brainmask_dir = join(out_root_dir, "ct_reg2_mr_based_on_betimg")
    os.makedirs(reg_bet_ct_brainmask_dir, exist_ok=True)
    reg_bet_ct_brainmask_file = join(reg_ct_brainmask_dir, fn.replace('.nii.gz', '_skullstrip_mask.nii.gz'))
    ants.image_write(reged_bet_ct_brainmask, reg_bet_ct_brainmask_file)
